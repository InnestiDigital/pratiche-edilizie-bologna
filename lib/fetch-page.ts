 ...
 * HTTP status, and content-type/body validation — and hands the raw JSON to the
 * ingress schema ({@link parseApiResponse}) for shape validation. It deliberately
 * knows nothing about SQLite, datasets, or paging so it can be unit-tested
 * without any Expo/native dependency.
 */
 ...
 *   envelope shape is wrong (re-thrown from {@link parseApiResponse}).
